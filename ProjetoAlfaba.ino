...
void loop() {
  ...
  for (uint8_t reader = 0; reader < NR_OF_READERS; reader++) {
    ...
    if (mfrc522[reader].PICC_IsNewCardPresent() && mfrc522[reader].PICC_ReadCardSerial()) {
      ...
      vetorString[reader].concat(String(mfrc522[reader].uid.uidByte < 0x10 ? " 0" : " "));
      vetorString[reader].concat(String(mfrc522[reader].uid.uidByte, HEX));
      //dump_byte_array(mfrc522[reader].uid.uidByte, mfrc522[reader].uid.size, vetorString[reader]);
      Serial.println();
      printPalavra(vetorString[reader]);
...
*/
void dump_byte_array(byte * buffer, byte bufferSize, String palavra) 
{
  for (byte i = 0; i < bufferSize; i++) {
    Serial.print(buffer[i] < 0x10 ? " 0" : " ");
    Serial.print(buffer[i], HEX);
    palavra.concat(String(buffer[i] < 0x10 ? " 0" : " "));
    palavra.concat(String(buffer[i], HEX));
  }
}

void printPalavra(String palavra){
  Serial.println("PRINT DA PALAVRA!");
